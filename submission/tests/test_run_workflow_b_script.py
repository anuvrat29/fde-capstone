"""Tests for submission/scripts/run_workflow_b.py, the one-command launcher
...
SCRIPT = ROOT / "submission" / "scripts" / "run_workflow_b.py"
...
class TestRunWorkflowBScript(unittest.TestCase):
    def test_successful_run_exits_zero_and_prints_valid_json(self):
        result = _run(["--fixture", "evaluation/fixtures/EV-06/fixture.json"])
        self.assertEqual(result.returncode, 0, result.stderr)
        payload = json.loads(result.stdout)
        self.assertEqual(payload["workflow"], "underwriting_pricing_support")
        self.assertEqual(payload["case_id"], "EV-06")
    ...
    def test_out_flag_writes_file(self):
        out_path = ROOT / "submission" / "tests" / "_scratch_ev06_output.json"
        try:
            result = _run([
                "--fixture", "evaluation/fixtures/EV-06/fixture.json",
