"""Tests for submission/scripts/run_workflow_a.py, the one-command launcher
...
SCRIPT = ROOT / "submission" / "scripts" / "run_workflow_a.py"
...
class TestRunWorkflowAScript(unittest.TestCase):
    def test_successful_run_exits_zero_and_prints_valid_json(self):
        result = _run(["--fixture", "evaluation/fixtures/EV-01/fixture.json"])
        self.assertEqual(result.returncode, 0, result.stderr)
        payload = json.loads(result.stdout)
        self.assertEqual(payload["workflow"], "coverage_claim_reconciliation")
        self.assertEqual(payload["case_id"], "EV-01")

    def test_wrong_workflow_fixture_fails_closed(self):
        result = _run(["--fixture", "evaluation/fixtures/EV-06/fixture.json"])  # underwriting_pricing_support
    ...
    def test_out_flag_writes_file(self):
        out_path = ROOT / "submission" / "tests" / "_scratch_ev01_output.json"
        try:
            result = _run([
                "--fixture", "evaluation/fixtures/EV-01/fixture.json",
