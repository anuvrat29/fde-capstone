...
| No tests (submission/tests) | Cannot demonstrate the negative/security tests referenced across all artefacts, including CTRL-RAF-02/03 (18), and the gate-2 tenant-isolation test newly identified in 21 | Author tests alongside first implementation increment |
| Non-compensable gate 8 (fixture traceability) has zero sub-claims in the assurance case (21) since template 22 does not yet exist | This is the least-assured of the 8 gates as of this log | Draft template 22 (Evaluation and TEVV) next, then immediately revisit 21_ASSURANCE_CASE.md to add gate 8's sub-claims |
| Non-compensable gate 2 (tenant isolation) has no specified multi-tenant test scenario anywhere in the submission — newly identified by the assurance case (21) | This is a genuine, previously-undetected gap, not merely deferred implementation | Escalate to CISO to specify a concrete multi-tenant test scenario |
...
| ISO/IEC 42001 Retirement-lifecycle-stage gap is now closed at the specification level (27), but 20_ISO42001_GOVERNANCE.md's own Open issues table has not yet been updated to reflect this closure | 20's text still describes the gap as open even though 27 resolves it | Update 20_ISO42001_GOVERNANCE.md's Open issues table to cite 27 as the closing artefact |
...
| Key management is identified as the highest-severity single point of failure in the stack-concentration finding (27), but no specific fallback vendor/mechanism has been evaluated | Losing key-management access could block recovery of five other dependent capabilities simultaneously | Escalate to CISO for a dedicated key-management continuity assessment |
| No operational runbooks exist yet for any of the three workflows' newly-named operational owners (26) | Naming an owner does not itself produce the runbook that owner would need | Author runbooks alongside first implementation increment |
| REQ-NF-12/13/16/17 have an architectural home (10), a governance taxonomy/design (13), and REQ-NF-13's cost half is now numerically framed (23) and REQ-NF-17's triggers are now SLI-specified (24), but none has an actual implementation yet | Bounded steps (traversal-depth ceiling), kill switch and full degraded-mode procedures remain undesigned at the final parameter level | Set remaining numeric values (traversal depth, cost ceiling) during implementation; author manual-mode procedures per 13/24's deferrals |
| Non-compensable gate 8 now has 9 representative (execution-pending) sub-claims via 22_EVALUATION_TEVV.md, but must be cross-referenced back into 21_ASSURANCE_CASE.md, which is not yet done | 21's own gate-8 claim currently still reads "gap" until this cross-reference is added | Update 21_ASSURANCE_CASE.md's gate 8 claim immediately, citing 22's test rows |
| Gate 2's tenant-isolation gap now has a concrete fixture identified (EV-17, per 22) but the test itself has not been executed | 21's gate-2 claim strength should be upgraded from "no scenario identified" to "scenario identified, not yet executed" | Update 21_ASSURANCE_CASE.md's gate 2 claim to reflect EV-17's identification |
| DOWN-01 (PolicyCore-IN) and DOWN-03 (AI-Primary-EU) have no recorded end timestamp and are treated as still-ongoing incidents (24); current actual status is unknown to this submission | Any workflow design assuming these systems are available would be relying on stale information | Escalate to CISO/Group Chief Claims Officer for a current-status confirmation before any deployment decision |
| The OM-Large 65% price increase (INJ-089, 23) is already effective (2026-08-15) and has no chosen budget response yet (absorb/substitute/renegotiate) | Delay risks silently absorbing the full increase by default | Escalate to Finance/AI Product Owner for an immediate decision before the effective date |
| D09 injects INJ-059 (bancassurance mis-selling), INJ-060 (broker remuneration conflict), INJ-062 (complaint clock mismatch) remain entirely unexamined | Conduct-risk rubric dimension partially touched (18's dark-pattern/accessibility angle) but not these three | Sequence into templates 25–29 if they surface additional conduct-risk implications |
| Graph-schema-level design for the fraud-ring component (INJ-045) has an architectural home (10), a read contract (12), and a denial-of-wallet threat case (16) but no query/schema-level design yet | None of 08/10/12/16 specified the actual graph schema or query language | Carry detailed schema design into the implementation pass |
| PayFlow read-only/propose-only boundary (ADR-12, CTRL-C4-01, CTRL-IC-04) is architecturally and contractually specified but not technically access-controlled | Non-compensable-gate-adjacent; must be enforced in code, not only in diagrams/contracts, before any user-facing pilot | Enforce during Workflow A/C implementation; verify with CTRL-C4-01/CTRL-IC-04 tests |
| Three pattern-specific controls in 14_INSURANCE_CONTROL_BOUNDARIES.md (reserve pre/post-value logging, payee-change-timing trigger, hardship-advance override justification) have no numeric parameters or accountable-role confirmation yet | INJ-027/036/048 each need a specific threshold or role before the control is enforceable, not just specified | Set thresholds with CISO/Chief Actuary/Group CEO input during implementation; confirm Legal/Recovery role for INJ-028 |
| TR-2026-CY-02's signed treaty wording is still pending (`slip_signed_wording_pending`) | The treaty-wording-resolution contract (12) and treaty-placement-authority row (14) can only apply a provisional-interpretation label until this is resolved | Monitor for signed-wording availability; re-evaluate both artefacts' provisional labelling when it arrives |
| The idempotency-ledger design for checkpoint replay (16, CTRL-TAM-02) and the consent-propagation window / retention-period matrix (17, A-03/A-04) have no numeric or storage-design specification yet | Each requires an implementation-time parameter or schema decision before the specified control is enforceable | Set with CISO/Data Governance Owner/Legal input during implementation |
| CatVision has no concrete incident-based SLO (24); only a general vendor-outage caveat exists | No numeric basis exists yet for this system's reliability target | Set once a concrete downtime event is recorded |
